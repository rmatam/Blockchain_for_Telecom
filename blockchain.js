...
Blockchain.prototype.createNewBlock = function(nonce,prevousblockHash,hash){
	const newBlock = {
		index: this.chain.length+1,
		timestamp: Date.now(),
		transcatios: this.pendingTransactions,
		nonce: nonce,
		hash: hash,
		prevousblockHash: prevousblockHash
	};
	this.pendingTransactions = [];
...
Blockchain.prototype.chainIsValid = function(blockchain) {
	...
	for(var i=1;i<blockchain.length;i++){
		...
		if (currentBlock['prevousblockHash'] !== prevBlock['hash']) validChain = false; //chain not valid
	...
	const correctTransactions = genesisBlock['transcatios'].length === 0;
